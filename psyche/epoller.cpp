...
void Epoller::add(const int fd) const {
    ...
    ev.data.fd = fd;
    ev.events = 0;
    check_errno(epoll_ctl(epollfd_, EPOLL_CTL_ADD, fd, &ev));
}

void Epoller::remove(const int fd) const {
    check_errno(epoll_ctl(epollfd_, EPOLL_CTL_DEL, fd, nullptr));
}
...
void Epoller::update(const int fd, const int events) const {
    ...
    ev.data.fd = fd;
    ev.events = events;
    check_errno(epoll_ctl(epollfd_, EPOLL_CTL_MOD, fd, &ev));
}
